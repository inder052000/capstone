...
def delete_values():
    for sn in list:
        sheet1 = client.open("database").worksheet(sn+4)
        existing = gd.get_as_dataframe(sheet1)
        sheet1_temp = client.open("database").worksheet(sn)
        existing_temp = gd.get_as_dataframe(sheet1_temp)
        sheet1_temp.clear()
        updated = existing.append(existing_temp)
...
#         k += 1  # i.e. at 30 fps, this advances one second
